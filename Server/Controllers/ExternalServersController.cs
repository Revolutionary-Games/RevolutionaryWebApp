...
public class ExternalServersController : Controller
{
    ...
    public async Task<IActionResult> Create([Required] [FromBody] ExternalServerDTO request)
    {
        ...
        await database.AdminActions.AddAsync(
            new AdminAction($"New external server with IP {request.PublicAddress} added")
            {
    ...
    public async Task<IActionResult> Destroy(long id)
    {
        ...
        await database.AdminActions.AddAsync(new AdminAction($"External server {id} deleted")
        {
    ...
    public async Task<IActionResult> MarkOffline(long id, [Required] bool offline)
    {
        ...
        if (offline)
        {
            ...
            await database.AdminActions.AddAsync(new AdminAction($"External server {id} marked offline")
            {
                PerformedById = user.Id,
            });
        }
        else
        {
            ...
            await database.AdminActions.AddAsync(new AdminAction($"External server {id} marked online")
            {
                PerformedById = user.Id,
            });
        }
    ...
    public async Task<IActionResult> ForceRebootServer(long id)
    {
        ...
        {
            await database.AdminActions.AddAsync(
                new AdminAction($"External server {id} force rebooted by an admin while it was reserved")
                {
                    PerformedById = user.Id,
                });
        }
        ...
        {
            await database.AdminActions.AddAsync(
                new AdminAction($"External server {id} force rebooted by an admin while it was provisioning")
                {
                    PerformedById = user.Id,
                });
        }
        ...
        {
            await database.AdminActions.AddAsync(new AdminAction($"External server {id} rebooted by an admin")
            {
                PerformedById = user.Id,
            });
        }
    ...
    public async Task<IActionResult> QueueMaintenance(long id)
    {
        ...
        await database.AdminActions.AddAsync(new AdminAction($"Server {id} is queued for maintenance")
        {
            PerformedById = HttpContext.AuthenticatedUserOrThrow().Id,
        });
